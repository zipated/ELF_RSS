...
```dotenv
AI_API_KEY="sk-xxx"                    # 必填，API 密钥
AI_API_BASE_URL="https://api.openai.com/v1"  # 可选，API 地址
AI_API_MODEL="gpt-4o-mini"             # 可选，模型名称
AI_API_PROMPT=""                        # 可选，自定义 system prompt，{lang} 为原语言变量
AI_API_EXTRA_PARAMS={}                  # 可选，额外参数，JSON 格式
AI_API_USE_PROXY=false                  # 可选，是否通过代理访问 API（需先配置 RSS_PROXY）
AI_API_KB_ENABLED=true                  # 可选，是否允许用于资料库生成（默认 true）
AI_API_ALLOWED_PARAMS=[]                # 可选，provider 参数白名单，JSON 数组
```

model 字段使用 LiteLLM 格式，常见示例：
- `openai/gpt-4o-mini`、`openai/gpt-4o`
- `anthropic/claude-3-5-sonnet`
- `deepseek/deepseek-chat`
- `ollama/llama3`（需配置 `AI_API_BASE_URL="http://localhost:11434"`）

##### 多节点配置

支持配置多个 API 节点，按顺序尝试直到成功。`AI_API_KEY` 作为默认节点，编号节点追加在其后：

```dotenv
AI_API_KEY="sk-xxx"
AI_API_BASE_URL="https://api.openai.com/v1"
AI_API_MODEL="gpt-4o-mini"
AI_API_USE_PROXY=true

AI_API_KEY1="sk-yyy"
AI_API_BASE_URL1="https://api.deepseek.com"
AI_API_MODEL1="deepseek/deepseek-chat"
AI_API_USE_PROXY1=false

AI_API_KEY2="sk-zzz"
AI_API_BASE_URL2="https://api.anthropic.com"
AI_API_MODEL2="anthropic/claude-3-5-sonnet"
AI_API_USE_PROXY2=true

...
```

每个节点可独立配置 `AI_API_KEY{n}`、`AI_API_BASE_URL{n}`、`AI_API_MODEL{n}`、`AI_API_PROMPT{n}`、`AI_API_EXTRA_PARAMS{n}`、`AI_API_USE_PROXY{n}`、`AI_API_KB_ENABLED{n}`、`AI_API_ALLOWED_PARAMS{n}`。

##### 代理配置

当某个节点的 `AI_API_USE_PROXY` 设为 `true` 时，该节点的 API 请求会通过 `RSS_PROXY` 代理访问。不同节点可独立设置是否走代理，适合以下场景：

- 默认节点（如 OpenAI）走代理，国内节点（如 DeepSeek）直连
- 多个国外 API 节点均走代理

注意：代理设置通过 `HTTP_PROXY` / `HTTPS_PROXY` 环境变量传递给 LiteLLM，调用完成后自动恢复。

##### 资料库生成控制

`AI_API_KB_ENABLED` 控制该节点是否用于资料库生成。某些模型功能不足（如缺少 reasoning 能力）可能导致资料库生成失败或质量差，可设为 `false` 跳过：

```dotenv
AI_API_KB_ENABLED=true   # 默认，允许用于资料库生成
AI_API_KB_ENABLED1=false  # 节点1 不参与资料库生成
```

##### Provider 参数放行

litellm 默认会拒绝非 OpenAI 标准参数（如 `thinking`），遇到 `UnsupportedParamsError` 时可配置 `AI_API_ALLOWED_PARAMS` 强制放行。不填则使用 litellm 默认行为。

```dotenv
AI_API_ALLOWED_PARAMS=["thinking", "reasoning_effort"]  # 强制放行这些参数
AI_API_ALLOWED_PARAMS4=[]   # 不填等同于不传，使用 litellm 默认行为
```
